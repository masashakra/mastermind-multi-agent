"""
Coopetition Peer-to-Peer — HTTP Servers for 4 agents (2 per team)

Team A: AnalyzerStrategist_A, Proposer_A
Team B: AnalyzerStrategist_B, Proposer_B

Each agent runs on a unique port and registers with the registry.
Teams communicate directly with each other via A2A (no Judge mediator).
"""
...
from paradigms.coopetition_peer_to_peer.agents.analyzer_strategist import AnalyzerStrategistAgent
from paradigms.coopetition_peer_to_peer.agents.proposer import ProposerAgent
...
def create_agent_server(agent_class, agent_id: str, team: str, port: int, provider: str, registry_url: str):
    ...
    agent = agent_class(
        ...
        paradigm=ParadigmType.COOPETITION_PEER_TO_PEER,
...
def _register_agent_with_registry(registry_url: str, agent_id: str, agent_url: str) -> None:
    ...
    try:
        card = {
            ...
            "paradigm": "coopetition_peer_to_peer",
            "url": agent_url,
            "capabilities": [],
            "constraints_owned": [],
            "team_members": [],
