#!/usr/bin/env python3
"""
Aggregate and report on 30 easy round-table semantic analyses (MM_001-MM_030).
"""
...
def load_analyses(logs_dir):
    """Load all semantic analysis JSON files for puzzles 001-030."""
    analyses = {}
    # Get all semantic analysis files matching the pattern
    analysis_files = list(logs_dir.glob("MM_*_round_table_deepseek_messages_semantic_analysis.json"))
    print(f"Found {len(analysis_files)} round-table analysis files total")

    for f in sorted(analysis_files):
        # Extract puzzle_id like MM_001 from MM_001_round_table_deepseek_messages_semantic_analysis.json
        puzzle_id = "_".join(f.name.split("_")[:2])
        puzzle_num = int(puzzle_id.split("_")[1])

        # Only include puzzles 001-030
        if puzzle_num <= 30:
            with open(f) as fp:
...
def print_report(analyses, stats):
    ...
    print("\n" + "="*80)
    print("  ROUND-TABLE EASY 30: SEMANTIC ANALYSIS AGGREGATE REPORT")
    print("="*80)
...
def main():
    ...
    # Load only easy 30 analyses
    analyses = load_analyses(logs_dir)

    if not analyses:
        print("No semantic analysis files found. Run analyze_message_pairs.py on round-table logs first.")
        return
    ...
    out_path = "logs/round_table_easy30_semantic_analysis_aggregate.json"
